...
Terrain::Terrain()
{
	...
	heightMap = new Texture("..\\resources\\heightMap.jpg", 0);
	groundTexdDiff = new Texture("..\\resources\\forest_diffuse.jpg", 1);
...
void Terrain::render(Shader* shader)
{
	...
	shader->use();
	shader->setInt("heightMap", heightMap->getTexUnit());
	glBindTexture(GL_TEXTURE_2D, heightMap ->getID());
	glActiveTexture(GL_TEXTURE0);

	/*shader->setInt("groundTex", groundTexdDiff->getTexUnit());
	glBindTexture(GL_TEXTURE_2D, groundTexdDiff->getID());
